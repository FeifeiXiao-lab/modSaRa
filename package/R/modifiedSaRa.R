...
#' @export
modifiedSaRa <-function(Y, alpha = 0.01, L = 100, h1 = 5, h2 = 10, h3 = 15, precise = 10000, simT = 100, sigma = NULL) {
  newcp  <-  h  <-  cnv.state <- cnv.start <-  cnv.end   <- vector()
  h.s = c(h1, h2, h3)
  FINV      <- vector("list", length(h.s))
  for (i in 1 : 3) {
    FINV [[i]] = fInverse(n = length(Y), h = h.s[i], hh = 2 * h.s[i], precise = precise, simT = simT)
  }
